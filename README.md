...
Определите перменную окружения `ROLLBAR_KEY`. Переменная хранит ключ для отправки исключений на внешний сервер https://rollbar.com. Для получения ключ зарегистрируйтесь на сайте и создайте новый проект. Устанвите значение перменной равной токену `post_server_item` в настрйоках проекта
